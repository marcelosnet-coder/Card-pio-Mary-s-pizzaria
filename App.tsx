...
const ProductCard: React.FC<{ 
  ...
  <div 
    ...
    className="bg-zinc-900/40 rounded-xl overflow-hidden shadow-lg border border-zinc-800 transition-all hover:border-orange-500 active:scale-95 cursor-pointer flex flex-col group"
  >
    <img src={product.imageUrl} alt={product.name} className="h-40 w-full object-cover group-hover:scale-105 transition-transform duration-500" />
...
export default function App() {
  ...
  return (
    <div className="min-h-screen pb-24 max-w-md mx-auto relative bg-black">
      {/* Header com fundo preto sólido para destacar a logo */}
      <header className="pt-10 pb-6 px-6 text-center bg-black">
        <div className="mb-4">
           <img 
             src={PIZZARIA_CONFIG.logoUrl} 
             alt="Mary's Pizzaria Logo" 
             className="w-48 h-48 object-contain mx-auto transition-transform hover:scale-105 duration-300"
           />
        </div>
      ...
      {view === 'menu' && (
        <div className="px-6 mb-8">
          <div className="relative rounded-2xl overflow-hidden h-48 shadow-2xl border border-zinc-900">
            <img 
      ...
      {lastOrder && view === 'menu' && cart.length === 0 && (
        <div className="mx-6 mb-6 p-4 bg-orange-600/10 border border-orange-600/20 rounded-xl flex items-center justify-between">
          <span className="text-xs text-orange-200">Deseja repetir o último pedido?</span>
          <button 
            onClick={repeatLastOrder}
            className="bg-orange-600 px-3 py-1 rounded-lg text-[10px] font-black uppercase transition-colors hover:bg-orange-700"
          >
            Repetir
          </button>
        </div>
      ...
      <main className="px-6">
        ...
        {view === 'selection' && (
          <div className="fixed inset-0 z-50 bg-black p-6 overflow-y-auto">
             <div className="flex items-center justify-between mb-8">
               ...
               <button onClick={() => setView('menu')} className="text-zinc-500 font-bold hover:text-white transition-colors">Voltar</button>
             </div>

             <div className="flex bg-zinc-900 p-1 rounded-xl mb-8">
               <button 
                 onClick={() => { setFlavorMode(1); setSelectedFlavors(prev => prev.slice(0, 1)); }}
                 className={`flex-1 py-3 rounded-lg font-black text-[10px] uppercase transition-all ${flavorMode === 1 ? 'bg-orange-600 text-white shadow-lg' : 'text-zinc-500'}`}
               >
                 1 Sabor
               </button>
               <button 
                 onClick={() => setFlavorMode(2)}
                 className={`flex-1 py-3 rounded-lg font-black text-[10px] uppercase transition-all ${flavorMode === 2 ? 'bg-orange-600 text-white shadow-lg' : 'text-zinc-500'}`}
               >
                 2 Sabores
               </button>
             </div>
             ...
             <div className="relative w-64 h-64 mx-auto mb-10">
                <div className="absolute inset-0 border-8 border-orange-950 rounded-full shadow-[0_0_30px_rgba(234,88,12,0.1)]"></div>
                {flavorMode === 1 ? (
                  <div className="w-full h-full rounded-full overflow-hidden">
                    {selectedFlavors[0] ? (
                      <img src={selectedFlavors[0].imageUrl} className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full bg-zinc-900 flex items-center justify-center text-zinc-600 text-[10px] p-8 text-center uppercase font-black">Escolha o sabor</div>
                    )}
                  </div>
                ) : (
                  <div className="w-full h-full rounded-full overflow-hidden flex">
                    <div className="w-1/2 h-full border-r-2 border-orange-950">
                      {selectedFlavors[0] ? (
                        ...
                        <div className="w-full h-full bg-zinc-900 flex items-center justify-center text-[8px] p-2 text-center text-zinc-600 uppercase font-black">Sabor 1</div>
                      )}
                    </div>
                    <div className="w-1/2 h-full">
                      {selectedFlavors[1] ? (
                        ...
                        <div className="w-full h-full bg-zinc-900 flex items-center justify-center text-[8px] p-2 text-center text-zinc-600 uppercase font-black">Sabor 2</div>
                      )}
                    </div>
                  </div>
                )}
                ...
                {selectedFlavors.length > 0 && (
                  <div className="absolute -top-2 -right-2">
                    <button 
                      ...
                      className="bg-red-600 text-white w-8 h-8 rounded-full flex items-center justify-center shadow-lg border-2 border-black font-bold active:scale-90 transition-transform"
                    >
                      ✕
                    </button>
                  </div>
             ...
             {selectedFlavors.length > 0 && (
                <div className="text-center mb-6">
                   <div className="inline-block px-4 py-2 bg-yellow-500 text-black font-black text-lg rounded-full shadow-lg">
                      R$ {(flavorMode === 1 
                        ? selectedFlavors[0].price 
                        : Math.max(selectedFlavors[0].price, selectedFlavors[1]?.price || 0)
                      ).toFixed(2).replace('.', ',')}
             ...
             <div className="space-y-4 mb-32">
               <h3 className="text-zinc-500 font-black uppercase text-[10px] tracking-widest pl-1">Opções:</h3>
                   ...
                   return (
                    <button
                      ...
                      className={`flex items-center p-3 rounded-xl border transition-all ${
                        ...
                        : (isFull && !isSelected) ? 'opacity-30 border-zinc-900' : 'bg-zinc-900 border-zinc-800 hover:border-zinc-600'
                      }`}
                    >
             ...
             <div className="fixed bottom-0 left-0 right-0 p-6 bg-black border-t border-zinc-900">
               <div className="max-w-md mx-auto space-y-4">
                 <textarea 
                    placeholder="Observações (ex: Sem cebola...)"
                    value={observations}
                    onChange={(e) => setObservations(e.target.value)}
                    className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-sm focus:outline-none focus:ring-1 focus:ring-orange-600 placeholder:text-zinc-600"
                    rows={2}
                 />
                 <button 
                   disabled={(flavorMode === 1 && selectedFlavors.length === 0) || (flavorMode === 2 && selectedFlavors.length < 2)}
                   onClick={finalizeFlavorSelection}
                   className="w-full bg-orange-600 disabled:bg-zinc-800 disabled:text-zinc-600 text-white font-black py-4 rounded-xl uppercase tracking-widest text-xs shadow-xl active:scale-[0.98] transition-all"
                 >
                   Confirmar Pizza
                 </button>
               </div>
        ...
        {view === 'checkout' && (
          <div className="space-y-10 animate-in slide-in-from-bottom duration-300">
            ...
            <div className="bg-zinc-900/40 rounded-2xl p-6 border border-zinc-800 shadow-xl">
              ...
              <div className="space-y-4">
                {cart.map(item => (
                  <div key={item.id} className="flex justify-between items-start border-b border-zinc-800 pb-3">
                    ...
                    <button onClick={() => removeFromCart(item.id)} className="text-red-500 font-bold active:scale-90 transition-transform">✕</button>
                  </div>
                ))}
            ...
            <div className="space-y-4">
               <h3 className="text-zinc-500 font-black uppercase text-[10px] tracking-widest pl-1">Entrega:</h3>
               <input 
                 type="text" 
                 placeholder="Nome Completo" 
                 value={customer.name}
                 onChange={(e) => setCustomer({...customer, name: e.target.value})}
                 className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-4 focus:outline-none focus:ring-1 focus:ring-orange-600 placeholder:text-zinc-700"
               />
               <input 
                 type="tel" 
               ...
               <textarea 
                 ...
                 className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-4 focus:outline-none focus:ring-1 focus:ring-orange-600 placeholder:text-zinc-700 h-24"
                 rows={3}
               />
            </div>
            ...
            <button 
              ...
              className="w-full bg-green-600 hover:bg-green-700 text-white font-black py-4 rounded-xl shadow-[0_10px_30px_-10px_rgba(22,163,74,0.4)] flex items-center justify-center space-x-3 uppercase tracking-widest text-xs transition-all active:scale-[0.98]"
            >
      ...
      {view === 'menu' && cart.length > 0 && (
        <div className="fixed bottom-6 left-6 right-6 z-40 max-w-md mx-auto">
          <button 
            ...
            className="w-full bg-orange-600 hover:bg-orange-700 text-white p-4 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.5)] flex items-center justify-between animate-in fade-in slide-in-from-bottom duration-500 active:scale-95 transition-all"
          >
            <div className="flex items-center">
              <span className="bg-white text-orange-600 font-black w-7 h-7 flex items-center justify-center rounded-lg mr-3 text-xs shadow-inner">{cart.length}</span>
